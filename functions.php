<?php
  ...
  function login($email,$password,$mysqli){
	...
	if($stmt = $mysqli->prepare("SELECT id, username, password, salt FROM students WHERE email = ? LIMIT 1")){
    ...
    print_r(mysql_stmt_error_list($stmt));*/
		...
		$stmt->store_result();
		$stmt->bind_result($student_id,$username,$db_password, $salt); //Get the data from the query in these variables.
		$stmt->fetch();
		...
		if($stmt->num_rows == 1){
			...
			if(checkbrute($student_id, $mysqli) == true){
				...
				if($db_password == $password){
					...
					$student_id = preg_replace("/[^0-9]+/", "", $student_id); //XSS protection as we might print this value
 ...
 function login_check($mysqli) {
   ...
   if(isset($_SESSION['student_id'], $_SESSION['username'], $_SESSION['login_string'])) {
     $student_id = $_SESSION['student_id'];
     $login_string = $_SESSION['login_string'];
     $ip_adress = $_SERVER['REMOTE_ADDR']; //IP adress of the user
     $username = $_SESSION['username'];
     $user_browser = $_SERVER['HTTP_USER_AGENT']; // reperisce la stringa 'user-agent' dell'utente.
     if ($stmt = $mysqli->prepare("SELECT password FROM students WHERE id = ? LIMIT 1")) {
        $stmt->bind_param('i', $student_id); // esegue il bind del parametro '$student_id'.
